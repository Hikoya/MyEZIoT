<?php
	...
	{	
		...
		$sql = "SELECT * FROM ".$config['tablenamenode']." where username = '".$username."' AND column3 IS NOT NULL order by timestamp asc limit ".$numType." ";
		...
		if(mysqli_num_rows($query3) > 0)
		{
			...
			foreach($gatewaynoarray as $key => $value)
			{
					...
					$sql4 = "SELECT * FROM " . $value . " WHERE column3 is NOT NULL order by timestamp desc limit 1 ";
					...
					if(mysqli_num_rows($query4) > 0)
					{
						...
						if($result5 = mysqli_fetch_array($query5,MYSQLI_ASSOC))
						{
							...
							$columnname = $result5['column3'];
							$columnname = ucfirst($columnname);
						}
					
						$addressarray = array("$columnname : $column3  ", " Location: $address " , " Last Updated: $timestamp ");
